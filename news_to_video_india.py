...
{{
  ...
  "shot_list": [{{"phrase": "a short excerpt of script_en (Hindi) that this beat
      ...
      SPECIFIC visual for exactly this moment - be SPECIFIC and INDIA-CONTEXTUALIZED,
      not generic: if the phrase mentions police, say 'Indian police officer in
      uniform' not just 'police'; if it names a festival or deity (Durga Puja, Ganesh
      Chaturthi, etc.), say the actual deity/festival imagery (e.g. 'Durga idol Hindu
      festival pandal'); if it names a specific state or city, describe THAT place's
      recognizable culture, landmark, or map outline (e.g. 'Kerala backwaters
      houseboat', 'Punjab wheat fields Golden Temple', 'West Bengal Howrah bridge
      Kolkata'); otherwise a generic but still India-set visual ('Indian government
      building exterior', 'Indian city street traffic') - never a specific real named
      person's face/likeness", "prefer_ai_generation": true if this is a specific
      cultural/religious/regional concept real stock footage is unlikely to have (a
      deity, a festival, a specific local landmark or tradition), false if it's a
      generic scene stock footage realistically has (traffic, a building, a crowd, a
      flag)}}] - 4 to 7 beats covering script_en roughly start to end in order. This
      drives WHICH footage shows WHEN; visual_keywords above is only used as a
...
MIN_BEAT_DURATION = 2.5   # shortest a single shot_list beat's background segment can be
...
def fetch_beat_footage(beat: ScriptBeat) -> List[str]:
    ...
    if not beat.prefer_ai_generation:
        india_query = beat.visual if "india" in beat.visual.lower() else f"India {beat.visual}"
        for link in search_pexels_clips(india_query)[:2]:
...
def build_romanized_word_timings(devanagari_words: List[WordTiming], script_romanized: str) -> List[WordTiming]:
...
def process_single_story(news: NewsItem, slot_index: int) -> bool:
    ...
    try:
        ...
        caption_words = build_romanized_word_timings(words, pkg.script_romanized)
        caption_text = pkg.script_romanized if pkg.script_romanized else pkg.script_en
