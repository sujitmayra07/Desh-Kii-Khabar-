...
ENVIRONMENT SETUP
--------------------------------------------------------------------------------
!apt-get update && apt-get install -y ffmpeg fonts-noto-core
!pip install -q feedparser openai edge-tts "moviepy==1.0.3" pillow numpy requests \
...
TTS_VOICE                   = "hi-IN-MadhurNeural"    # or "hi-IN-SwaraNeural" (female)
TTS_RATE                    = "+8%"
VIDEO_WIDTH                 = 1080
VIDEO_HEIGHT                 = 1920
...
SLOT_WINDOWS = [
    (7, 33, 7, 47),     # Slot 1: ~7:40 AM
    (8, 53, 9, 7),      # Slot 2: ~9:00 AM
    (11, 4, 11, 18),    # Slot 3: ~11:11 AM
    (13, 4, 13, 18),    # Slot 4: ~1:11 PM
    (17, 53, 18, 7),    # Slot 5: ~6:00 PM
    (19, 23, 19, 37),   # Slot 6: ~7:30 PM
]
...
{{
  ...
  "script_en": "The full spoken voiceover script, in natural, conversational, spoken HINDI
      (Devanagari script - not Hinglish/Latin transliteration). STRICTLY 80 to 100 Hindi
      words so spoken audio lands between 30 and 40 seconds. Structure internally as:
      (0-3s) a scroll-stopping, curiosity-driven HOOK; (3-25s) fast-paced, factual
      storytelling, written so the visual background could reasonably cut to something
      new every 2-3 seconds; (25-35s) a distinct ANALYSIS beat in your own words - for a
      geopolitics/national story, explain what this means for India (bharat ke liye iska
      kya matlab hai); for a state/regional story, explain why it actually matters or
      what happens next for the people affected - either way this must read as genuine
      original commentary, not a repeated fact from the hook, and must not be one-sided
      political propaganda for or against any party, state, country, or group; (last 3-5s) a punchy call-to-action in Hindi
      telling viewers to like the video and subscribe. If the topic involves conflict or
      tragedy, stay factual and measured - no glorifying, mocking, or graphic language.
      Do NOT include stage directions, timestamps, or brackets - only words to be spoken
      aloud. If is_credible is false, still fill this with an empty string.",
  "title": "A highly attractive, curiosity-driven, clickbait-style video title in HINDI
      (Devanagari) with 1-3 emojis - optimized to stop the scroll, still accurate to the
      story.",
  "description": "An attractive, SEO-optimized description (2-4 sentences) for YT Shorts,
      in HINDI, written to rank in YouTube search for this story's topic.",
  "hashtags": ["10 to 15 high-ranking, SEO-friendly hashtags as strings, each starting
      with #, mixing Hindi-audience and English hashtags for maximum reach, e.g. #Shorts,
      #India, #Geopolitics, #Bharat, #News, #Trending"],
  "tags": ["10 to 15 plain SEO keyword phrases (no #) for the YouTube tags box - a mix of
      Hindi and English search terms real viewers would type"],
...
def find_caption_font() -> Optional[str]:
    """Hindi captions need a font with Devanagari glyph coverage - the Latin-
    only fonts (DejaVu/Liberation/Arial) the English v4 pipeline used will
    render Hindi text as blank/tofu boxes, not an error, just silently wrong
    output. Re-adds what that pipeline's own changelog says it deliberately
    dropped, since English captions didn't need it.

    Tries fontconfig's `fc-match` first (most portable - exact install paths
    vary by distro/package version), then known Noto/Lohit Devanagari paths
    (main.yml installs fonts-noto-core via apt so this should normally
    resolve on path 1 or 2 in CI), then a download fallback, then logs an
    actionable warning if all three fail rather than silently degrading."""
    try:
        ...
        if candidate and os.path.exists(candidate):
            log.info(f"  Using caption font (via fontconfig): {candidate}")
            return candidate
    except Exception as e:
    ...
    dest = os.path.join(TEMP_DIR, "NotoSansDevanagari-Bold.ttf")
    url = "https://github.com/notofonts/devanagari/raw/main/fonts/NotoSansDevanagari/hinted/ttf/NotoSansDevanagari-Bold.ttf"
    try:
        r = requests.get(url, timeout=REQUEST_TIMEOUT)
        r.raise_for_status()
        with open(dest, "wb") as f:
            f.write(r.content)
        log.info("  -> Downloaded NotoSansDevanagari-Bold font for Hindi captions.")
        return dest
    except Exception as e:
        log.warning(
            f"  Could not fetch a Devanagari fallback font ({e}). Hindi captions will "
            "likely render as blank boxes with PIL's default font. ACTION: confirm "
            "'fonts-noto-core' is installed in main.yml's apt-get step, or run "
            "`fc-list | grep -i devanagari` on the runner to see what's actually there."
        )
        return None
