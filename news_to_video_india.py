...
def find_caption_font() -> Optional[str]:
    ...
    CONFIRMED LIVE BUG in an earlier version of this function: the apt-path
    guesses and the original download URL both turned out to be unreliable
    in practice (captions rendered as boxes on real runs) - fonts-noto-core
    either wasn't providing Devanagari coverage under that exact path, or
    the original download URL (github.com/notofonts/devanagari/...) wasn't
    resolving. The download URL below has been directly verified to exist
    (fetched and confirmed a real ~138KB TTF, not assumed from a search
    result) - github.com/openmaptiles/fonts, a maintained, actively-used
    font-bundling repo, static (non-variable) weights, which also sidesteps
    a separate risk: PIL's ImageFont has had mixed support for VARIABLE-font
    weight-axis selection, and Noto's own current distribution is a variable
    font. The downloaded file is also now sanity-checked by actually loading
    it with PIL before trusting it, rather than trusting a 200 OK response
    alone - a response can be a 200 with an HTML error page body, not
    necessarily a real font."""
    try:
    ...
    for url in download_urls:
        ...
        try:
            ...
            log.info(f"  -> Downloaded and verified {os.path.basename(url)} for Hindi captions.")
            return dest
        except Exception as e:
...
def build_breaking_news_banner(topic_text: str, font_path: Optional[str], duration: float) -> ImageClip:
    ...
    draw.rectangle([0, 0, badge_w, badge_h], fill=(178, 24, 24, 255))
    draw.rectangle([badge_w, 0, w, badge_h], fill=(18, 38, 88, 255))
    f_badge = _fit_font(draw, "BREAKING NEWS", font_path, badge_w - 24, int(badge_h * 0.62))
    _draw_text_centered(draw, "BREAKING NEWS", f_badge, badge_h // 2, badge_w, (255, 255, 255, 255))
...
def process_single_story(news: NewsItem, slot_index: int) -> bool:
    ...
    try:
        ...
        assemble_final_video(bg_video, caption_clips, voice_path, final_path, banner_topic=pkg.title, font_path=font_path)
    except Exception:
